#ifndef DOUBLEHASHPROBINGHASH_H
#define DOUBLEHASHPROBINGHASH_H
...
class DoubleHashingProbingHash : public HashInterface<Key, Value> {
public:
    // constructor
    DoubleHashingProbingHash(int table_size, int double_f) : tableSize(table_size), doubleFactor(double_f), collisionsCount(0) {
    ...
    int lookUp(const Key& key) {
        ...
        collisionsCount++;
        int step = doubleHash_Function(key);
        int i = 1;

        int index = startIndex;
        while (true) {
            index = (startIndex + i * step) % data.size();
    ...
    void printTable(std::ofstream& fout) {
        ...
        fout << "\nDouble Hashing probing " << collisionsCount << " collisions\n" << std::endl;
    }

    virtual ~DoubleHashingProbingHash() override {} // data vector should automatically delete
...
private:
    ...
    int doubleHash_Function(const Key& key) {
        return doubleFactor - (hash_function(key) % doubleFactor);
    }
    
};

#endif // DOUBLEHASHPROBINGHASH_H
